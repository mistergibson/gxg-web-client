...
class MimeMagic
  ...
  def self.cast_to_ascii(the_string="")
    `new window.TextDecoder("ascii").decode(new window.TextEncoder().encode(#{the_string}.toString()))`.to_s
  end
  ...
  def self.magic_match(io, method)
    return magic_match(StringIO.new(cast_to_ascii(io.to_s)), method) unless io.respond_to?(:read)

    # io.binmode if io.respond_to?(:binmode)
    # io.set_encoding(Encoding::BINARY) if io.respond_to?(:set_encoding)
    # buffer = "".force_encoding(Encoding::BINARY)
    buffer = cast_to_ascii("")
